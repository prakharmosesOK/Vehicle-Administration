        ...
        try:
            ...
            def delete():
                try:
                    ...
                    if quantity=='0':
                        ...
                        qryft3="SELECT * FROM Left_food WHERE Id = %s"
                        crs.execute(qryft3,(Id,))
                        dataft3=crs.fetchall()
                        lftqty=str(float(dataft3[4]-float(quantity)))
                        
                        qry42="UPDATE Menu SET Quantity=%s WHERE Id = %s"
                        data42=(lftqty,f_idel)
            ...
            def generate_bill():
                try:
                    ...
                    pdf.cell(16,10,"S.No.           NAME_OF_FOOD_ITEM             COST_PER_PRODUCT         QUANTITY",ln=1,align="L")
                    ...
                    for i in myresult:
                        A.append(i)
                        pdf.cell(17,5,str(k),0,0,align="C")
            ...
            def display_sp():
                try:
                    ...
                    crs.execute(qry92)
                    data92=crs.fetchall()
                    df92=pd.DataFrame(data92,columns=['ID','Customer name','Food item','Type','Price','Food Quantity','Food Id'])
                    print(df92)
                    print()
                    print()
                    crs.close()
